class RMObjective : RMC
{
	...
	void RenderTimer() override
	{
		...
		if (RMC::IsRunning || RMC::RunEndTimestamp() > 0 || RMC::RunStartTimestamp > 0) {
			RunTime = RMC::RunStartTimestamp - RunTimeStart;
	...
	void TimerYield() override {
		while (RMC::IsRunning){
			...
			if (!RMC::IsPaused) {
				...
				if (currentMap !is null) {
					...
					if (currentMapInfo !is null) {
						if (DataJson["recentlyPlayed"].Length > 0 && currentMapInfo.MapUid == DataJson["recentlyPlayed"][0]["TrackUID"]) {
							RMC::RunStartTimestamp = Time::get_Now();
				...
				// pause timer
				RMC::RunStartTimestamp = Time::get_Now() - (Time::get_Now() - RMC::RunStartTimestamp);
#if DEPENDENCY_CHAOSMODE
			...
			if (RMC::GetCurrentMapMedal() >= RMC::Medals.Find(PluginSettings::RMC_GoalMedal)-1 && !RMC::GotGoalMedalOnCurrentMap && PluginSettings::RMC_GoalMedal != RMC::Medals[0]) {
