namespace RMC
{
	...
	void RenderRMCMenu()
	{
		...
		if (Permissions::PlayLocalMap()) {
			...
			if (Permissions::CreateActivity() && UI::TreeNode(MX_COLOR_STR + Icons::Users + " \\$zRandom Map Together \\$f33(BETA)")) {
				...
				if (RMT_isServerOK && TM::IsInServer() && UI::GreenButton(Icons::Heart + " Start RMT Survival")){
					...
					startnew(CoroutineFunc(TogetherSurvival.StartRMT));
				}
#endif
				UI::TreePop();
			}
		...
		if (RMC::GoalMedalCount > 0 || Challenge.BelowMedalCount > 0 || Survival.Skips > 0 || Survival.SurvivedTime() > 0) {
			...
			else if (selectedGameMode == GameMode::TogetherSurvival) {
				pos_orig = UI::GetCursorPos();
				TogetherSurvival.RenderGoalMedal();
				UI::SetCursorPos(vec2(pos_orig.x+100, pos_orig.y));
				TogetherSurvival.RenderBelowGoalMedal();
				UI::SetCursorPos(vec2(pos_orig.x, pos_orig.y+60));
				TogetherSurvival.RenderScores();
				UI::Text("Survived time: " + RMC::FormatTimer(0));
			}
#endif
		}
	}
